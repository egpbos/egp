...
#ifdef NUM_THREADS
extern pthread_mutex_t mutex_nexport, mutex_partdrift, mutex_nodedrift;

#define LOCK_NEXPORT         pthread_mutex_lock(&mutex_nexport);
#define UNLOCK_NEXPORT       pthread_mutex_unlock(&mutex_nexport);
#define LOCK_PARTICLEDRIFT   pthread_mutex_lock(&mutex_partdrift);
#define UNLOCK_PARTICLEDRIFT pthread_mutex_unlock(&mutex_partdrift);
#define LOCK_NODEDRIFT       pthread_mutex_lock(&mutex_nodedrift);
#define UNLOCK_NODEDRIFT     pthread_mutex_unlock(&mutex_nodedrift);
#else
#define LOCK_NEXPORT
#define UNLOCK_NEXPORT
#define LOCK_PARTICLEDRIFT
#define UNLOCK_PARTICLEDRIFT
#define LOCK_NODEDRIFT
#define UNLOCK_NODEDRIFT
#endif
...
#ifdef SFR 
		  if(pa->Type == 0)
		    if(((All.ComovingIntegrationOn == 1) && (SphP[p].d.Density / (All.Time * All.Time * All.Time) >= All.PhysDensThresh)) 
		       || ((All.ComovingIntegrationOn == 0) && (SphP[p].d.Density >= All.PhysDensThresh)))
		      {
			stellar_s[0] += (pa->Mass * pa->Pos[0]);
			stellar_s[1] += (pa->Mass * pa->Pos[1]);
			stellar_s[2] += (pa->Mass * pa->Pos[2]);
			stellar_mass += (pa->Mass);
			printf("ET sfr ID: %d Proc %d", pa->ID, ThisTask);
		      }
#endif
#endif
...
/*
  tidal_tensorps_temp[1][0] = tidal_tensorps_temp[0][1];
  tidal_tensorps_temp[2][0] = 0.0*tidal_tensorps_temp[0][2];
  tidal_tensorps_temp[2][1] = 0.0*tidal_tensorps_temp[1][2];
*/
