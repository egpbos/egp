	  ...
	  if(P[i].Type == 5)
	    {
	      ...
	      if (P[i].BH_Mass > 0) TimeBin_BH_Medd[binold] -= P[i].BH_Mdot / P[i].BH_Mass;
	      TimeBin_BH_mass[bin] += P[i].BH_Mass;
	      TimeBin_BH_dynamicalmass[bin] += P[i].Mass;
	      TimeBin_BH_Mdot[bin] += P[i].BH_Mdot;
	      if (P[i].BH_Mass > 0) TimeBin_BH_Medd[bin] += P[i].BH_Mdot / P[i].BH_Mass;
	    }
#endif
  ...
  if(All.PM_Ti_endstep == All.Ti_Current)	/* need to do long-range kick */
      ...
      for(i = 0; i < NumPart; i++)
	{
	  ...
	  for(j1 = 0; j1 < 3; j1++)
		...
		for(j = 0; j < 3; j++)
		  {
		    dv_distortion_tensorps[j1 + 3][j2]     += P[i].tidal_tensorpsPM[j1][j] * P[i].distortion_tensorps[j][j2];
		    dv_distortion_tensorps[j1 + 3][j2 + 3] += P[i].tidal_tensorpsPM[j1][j] * P[i].distortion_tensorps[j][j2 + 3];
		  }
...
void do_the_kick(int i, int tstart, int tend, int tcurrent)
{
  ...
  for(j1 = 0; j1 < 3; j1++)
	...
	for(j = 0; j < 3; j++)
	  {
	    dv_distortion_tensorps[j1 + 3][j2]     += P[i].tidal_tensorps[j1][j] * P[i].distortion_tensorps[j][j2];
	    dv_distortion_tensorps[j1 + 3][j2 + 3] += P[i].tidal_tensorps[j1][j] * P[i].distortion_tensorps[j][j2 + 3];
	  }
	dv_distortion_tensorps[j1 + 3][j2]     *= dt_gravkick;
	dv_distortion_tensorps[j1 + 3][j2 + 3] *= dt_gravkick;
				   ...
				   aphys */ )
{
  ...
  if(!(ti_step > 0 && ti_step < TIMEBASE))
	...
	printf("Cosmic Ray Properties: C0: %g -- q0  : %g -- P  : %g\n"
		 "                       Rho: %g\n\n",
		 SphP[p].CR_C0[CRpop], SphP[p].CR_q0[CRpop], CR_Particle_Pressure(SphP + p, CRpop), SphP[p].d.Density);
#endif
