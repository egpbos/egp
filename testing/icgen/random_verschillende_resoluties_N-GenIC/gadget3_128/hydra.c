...
int total_count=0;
  ...
  for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
	...
	/* DtEntropy stores the energy change rate in physical units */
#else
	/* DtEntropy stores the energy change rate in internal units */
#ifndef WAKEUP
	eos_calc_dsdt(SphP[i].temp, SphP[i].d.Density * All.UnitDensity_in_cgs, SphP[i].xnuc, SphP[i].u,
		      SphP[i].Entropy, SphP[i].e.DtEntropy * All.UnitEnergy_in_cgs / All.UnitTime_in_s,
		      (P[i].TimeBin ? (1 << P[i].TimeBin) : 0) * All.Timebase_interval * All.UnitTime_in_s,
		      &SphP[i].e.DtEntropy);
#else
	eos_calc_dsdt(SphP[i].temp, SphP[i].d.Density * All.UnitDensity_in_cgs, SphP[i].xnuc, SphP[i].u,
		      SphP[i].Entropy, SphP[i].e.DtEntropy * All.UnitEnergy_in_cgs / All.UnitTime_in_s,
		      P[i].dt_step * All.Timebase_interval * All.UnitTime_in_s, &SphP[i].e.DtEntropy);
#endif
	/* DtEntropy stores entropy change in physical units */
#endif
#endif
 ...
 int CRpop;
  ...
  if(P[i].TimeBin)	/* upon start-up, we need to protect against dt==0 */
	...
	phiphi = SphP[i].PhiPred * All.DivBcleanParabolicSigma * 0.5 * SphP[i].MaxSignalVel / (PPP[i].Hsml * fac_mu);
...
=======
		      magfac_sym *= vsig * 0.5 * Balpha_ij * r *   
                                    rho / (rho_ij ) ;
		      dtEntropy += dTu_diss_b * 0.25 * vsig * mu0_1 * r / (rho_ij );
=======
