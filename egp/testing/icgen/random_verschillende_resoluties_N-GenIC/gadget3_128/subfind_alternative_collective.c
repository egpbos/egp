...
void subfind_process_group_collectively(int num)
{
  ...
  Offset_NumPartGroup = mymalloc(NTask * sizeof(int));
  ...
  max_candidates = (TotalGroupLen / NTask / 100);
  ...
  if(ThisTask == 0)
    sd = mymalloc(TotalGroupLen * sizeof(struct sort_density_data));
  else
  ...
  if(ThisTask == 0)
    {
      int ncurrent = NumPartGroup;

      for(task = 1; task < NTask; task++)
	{
	  MPI_Send(&task, 1, MPI_INT, task, TAG_N, MPI_COMM_WORLD);
	  MPI_Recv(&sd[Offset_NumPartGroup[task]], List_NumPartGroup[task] * sizeof(struct sort_density_data), MPI_BYTE,
		   task, TAG_PDATA, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	}
    }
  else
    {
  ...
  if(ThisTask == 0)
    {
      for(i = NTask - 1; i >= 0; i--)
	{
	  myfree_msg(Tail[i], "Tail[i]");
	  myfree_msg(Head[i], "Head[i]");
	}

      myfree_msg(Tail, "Tail");
      myfree_msg(Head, "Head");
    }
  ...
  do
      ...
      if(ThisTask == 0)
	{
	  ...
	  myfree_msg(tmp_candidates, "tmp_candidates");
      ...
      if(ThisTask == 0)
	{
	  ...
	  origintask = mymalloc(NTask * sizeof(int *));
	  targettask = mymalloc(NTask * sizeof(int *));
	  submark = mymalloc(NTask * sizeof(int *));

	  for(task = 0; task < NTask; task++)
	    {
	      origintask[task] = mymalloc(List_NumPartGroup[task] * sizeof(int));
	      targettask[task] = mymalloc(List_NumPartGroup[task] * sizeof(int));
	      submark[task] = mymalloc(List_NumPartGroup[task] * sizeof(int));
	    }
      ...
      else
	{
	  ...
	  origin = mymalloc(NumPartGroup * sizeof(int));
	  target = mymalloc(NumPartGroup * sizeof(int));
	  mark = mymalloc(NumPartGroup * sizeof(int));
      ...
      else
	{
	  ...
	  loc_candidates = mymalloc(loc_count_cand * sizeof(struct cand_dat));
	  ...
	  for(task = 1; task < NTask; task++)
	    {
	      ...
	      int *submark = mymalloc(List_NumPartGroup[task] * sizeof(int));
	...
	{
	  int *submark = mymalloc(NumPartGroup * sizeof(int));
  ...
  if(ThisTask == 0)
    {
      List_LocalLen = mymalloc(NTask * sizeof(int));
      ud_list = mymalloc(NTask * sizeof(struct unbind_data *));

      for(i = 0; i < NTask; i++)
	ud_list[i] = mymalloc(List_NumPartGroup[i] * sizeof(struct unbind_data));
  ...
  if(ThisTask == 0)
    {
      for(i = NTask - 1; i >= 0; i--)
	{
	  myfree_msg(Len[i], "Len[i]");
	  myfree_msg(Next[i], "Next[i]");
	}

      myfree_msg(Len, "Len");
      myfree_msg(Next, "Next");

      for(i = NTask - 1; i >= 0; i--)
	myfree_msg(candidates[i], "candidates[i]");

      myfree_msg(candidates, "candidates");
      myfree_msg(count_cand, "count_cand");
    }

  myfree_msg(Offset_NumPartGroup, "Offset_NumPartGroup");
  myfree_msg(List_NumPartGroup, "List_NumPartGroup");
